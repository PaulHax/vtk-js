...
function vtkWidgetManager(publicAPI, model) {
  ...
  function deleteSelectorWhenReady(selector, captureInProgress) {
    if (!selector) {
      return;
    }
  ...
  async function updateSelection(callData, fromTouchEvent, callID) {
    ...
    const selectedData = await publicAPI.getSelectedDataForXY(
      position.x,
      position.y
    );

    if (publicAPI.isDeleted()) {
      return;
    }
  ...
  publicAPI.getSelectedDataForXY = async (x, y) => {
    ...
    if (model.pickingEnabled) {
      ...
      if (publicAPI.isDeleted() || !model._capturedBuffers) {
        return {};
      }
