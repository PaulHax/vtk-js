...
// gl-matrix defaults to Float32Array; use Float64Array here to match
// production precision and avoid round-trip error.
...
function projectWithWebGPU(coreCamera, webgpuCamera, worldPoint) {
  ...
  mat4.transpose(view, view); // Core's getViewMatrix is pre-transposed once
...
// Backend-agnostic half of picking, once handed standard 0=near,1=far
...
describe('WebGPU Camera explicit projection matrix', () => {
  ...
  describe.each([
    ...
    it('maps the near clipping plane to reversed-Z hardware depth 1, decreasing towards far', () => {
      ...
      // Both convertToOpenGLDepth branches must round-trip exactly at the
      // clipping planes.
    ...
    it('picks the same world position via the standard path and an equivalent explicit-matrix override', () => {
      ...
      // pre-transposes, so transposing again recovers the raw GL-style matrix.
